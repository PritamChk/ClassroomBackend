...
# class CollegeCreate3Serializer(_ms):
#     allowed_dbas = _sz.ListField(child=_sz.CharField())
#     allowed_teacher_list = _sz.FileField(max_length=None, use_url=True, required=False)
...
#         for email in dba_mails:
#             p(email, "cyan")

#         for dba_mail in dba_mails:
#             if AllowedCollegeDBA.objects.filter(email=dba_mail).exists():
#                 raise _error(
#                     detail=f"DBA --> {dba_mail} already associated with a college & cannot create a new college",
...
#                 AllowedCollegeDBA(college=college, email=email) for email in dba_mails
#             ]
#             AllowedCollegeDBA.objects.bulk_create(allowed_dba_object_list)
#         except:
#             raise _error(
...
# class CollegeCreate2Serializer(_wnms):
